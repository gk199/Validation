...
int main() {
  ...
  while (myReader.Next()){
	...
	if ( (frame[i].soi() == true) && (frame[i].adc() >= adc_of_interest) && (abs(QIEdetectorID.ieta()) == ieta_of_interest)) {
	  event_withADC.push_back(evtCounter);
        ...
        if ( (frame[i].soi() == true) && (frame[i].adc() >= adc_of_interest2) && (abs(QIEdetectorID.ieta()) == ieta_of_interest)) {
