// Macro to use the TMVA trained classifiers from mvaAnalysisTemplate_multiplicity.py for use on LLP samples with multiplicity / trigger algorithm calculated in HCAL, using timing and depth segmentation
/**********************************************************************************
 ...
 * from https://root.cern.ch/doc/v606/TMVAClassificationApplication_8C_source.html*
 * and https://root.cern.ch/doc/v610/TMVAClassificationApplication_8C.html        *
 **********************************************************************************/
...
void TMVAClassificationApplication( TString myMethodList = "" )
{
  ...
  /*
  Float_t spec1,spec2;
  reader->AddSpectator( "spec1 := x0*2",   &spec1 );
  reader->AddSpectator( "spec2 := x0*3",   &spec2 );
  */
  Float_t Category_cat1, Category_cat2, Category_cat3;
  if (Use["Category"]){
    // Add artificial spectators for distinguishing categories
    reader->AddSpectator( "Category_cat1 := x2<=0",             &Category_cat1 );
    reader->AddSpectator( "Category_cat2 := (x2>0)&&(x3<0)",  &Category_cat2 );
    reader->AddSpectator( "Category_cat3 := (x2>0)&&(x3>=0)", &Category_cat3 );
  }
  // Book the MVA methods
  TString dir    = "dataset/weights/";
  TString prefix = "TMVAClass";
  // Book method(s)
  for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
    if (it->second) {
  ...
  for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
    ...
    //    x0 = userVar1 + userVar2;
    //    x1 = userVar1 - userVar2;
