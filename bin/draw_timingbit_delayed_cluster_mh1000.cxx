...
int main()
{
  ...
  std::map<std::string, TH1F*> multHists_LLPpl100000;
  ...
  for(auto multType : multTypes) {
    ...
    multHists_LLPpl10000[multType] = dynamic_cast<TH1F*>(files.at(1)->Get(histName.c_str()));
    multHists_LLPpl100000[multType] = dynamic_cast<TH1F*>(files.at(2)->Get(histName.c_str()));
  }

  for(auto pair : multHists_QCD) pair.second->SetLineWidth(2);
  for(auto pair : multHists_LLPpl10000) pair.second->SetLineWidth(2);
  for(auto pair : multHists_LLPpl100000) pair.second->SetLineWidth(2);
