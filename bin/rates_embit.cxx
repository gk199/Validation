...
void rates(bool newConditions, const std::string& inputFileDirectory){
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++){
	...
	if (l1emu_->egTowerIEta[c] > 28){
	  for (int t=0; t<l1TPemu_->nHCALTP; t++){
	    if (l1TPemu_->hcalTPet[t] <= 0.5 or abs(l1TPemu_->hcalTPieta[t]) <= 28) continue;
	    double dR = deltaR(l1emu_->egEta[c], l1emu_->egPhi[c], etaVal(l1TPemu_->hcalTPieta[t] ), phiVal(l1TPemu_->hcalTPCaliphi[t] ));
	    if (dR <= 0.2)
	      {
		emfbit = emfbit or l1TPemu_->hcalTPfineGrain[t];
	      }
	  }
	  if (!emfbit) continue; // this should skip to the next eg
	}
