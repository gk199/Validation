...
void rates(bool newConditions, const std::string& inputFileDirectory){
  ...
  TH1F * DeltaR_TP_4L1Jets = new TH1F("DeltaR_TP_4L1Jets", "DeltaR Between Leading L1 Jets and Closest HCAL TP; DeltaR;Number of Events",12,0,0.8); // earlier binning was 20
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++){
	...
	if (closestJet == 0) DeltaR_TP_4L1Jets->Fill(min_DR_L1_TP);
	...
	if (closestJet == 3) DeltaR_TP_L1Jet_4->Fill(min_DeltaR);
        ...
        if ( min_DeltaR > 0.6 ) continue; // don't fill matched multiplicity for TPs that are DR > 1 away from their nearest L1 jet         
	...
	for (int depthIt = 0; depthIt < nDepth-1; depthIt++){
	  ...
	  if ( (hcalTPdepth[depthIt] > 3) && (hcalTPtiming[depthIt] > 3) && (abs(tpEtaemu) < 16) ) { // 3 GeV 3ns in HB region
	    ...
	    if (closestJet == 3) mult3GeV3nsHB_Jet3 += 1;
          ...
          // multiplicity in 1,2,3 GeV 1,2,3,4,5 ns HB, HE, jet matched regions
	  if (hcalTPdepth[depthIt] > 3 ){
      ...
      //      DeltaR_TP_4L1Jets->Fill(min_DR_L1_TP); // min distance between HCAL TP and L1 jet per event

      int mult3GeV3nsHB_nearL1Jets_total = mult3GeV3nsHB_Jet0 + mult3GeV3nsHB_Jet1 + mult3GeV3nsHB_Jet2 + mult3GeV3nsHB_Jet3;
