...
int main() {
  ...
  while (myReader.Next()){
    ...
    for (QIE11DataFrame frame:*FullQIE11DataFrame) { // loop over QIE11 data frame in HcalDataFrameContainer, this goes over FullQIE11DataFrame->size()
      ...
      if ( (QIEdetectorID.ieta() == ieta_of_interest) && (QIEdetectorID.depth() == depth_of_interest) ) { // (QIEdetectorID.iphi() == iphi_of_interest) && (QIEdetectorID.depth() == depth_of_interest) ) {
    ...
    for (CaloSamples CaloSample:*AllCaloSamples) { // loop over everything in AllCaloSamples, call it CaloSample
      ...
      if ( (detectorID.ieta() == ieta_of_interest) && (detectorID.depth() == depth_of_interest) ) { // (detectorID.iphi() == iphi_of_interest) && (detectorID.depth() == depth_of_interest) ) { // for a particular ieta, iphi, depth pulse shape
