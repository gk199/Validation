...
void efficiency_ADC_combined(){
  ...
  h1->SetTitle("Delayed Jet ADC Value Efficiency");
  ...
  legend1->AddEntry(pEff1, "Delayed Hit", "l");
  legend1->AddEntry(pEff2, "Delayed Hit, event HT>120", "l");

  legend1->Draw("same");

  char saveFile[100];
  sprintf(saveFile,"/eos/user/g/gkopp/www/HCAL_LLP/TimingBit/112X_TDCsim_DelayedJet/ADCefficiency_4combined.pdf");
  c1->SaveAs(saveFile);
